...
def create_user(client: TestClient, n: int):
    payload = {
        "email": f"postuser{n}@example.com",
        "username": f"postuser{n}",
...
def test_post_permissions(client: TestClient):
    ...
    res = client.post(
        "/api/v1/posts/", json={"title": "A's post", "content": "hello"}, headers=headers_A
    )
    post_id = res.json()["id"]
...
def test_post_feed(client: TestClient):
    ...
    res = client.post(
        "/api/v1/posts/", json={"title": "B Post", "content": "hi"}, headers=headers_B
    )
    post_id = res.json()["id"]
...
def test_get_single_post(client: TestClient):
    ...
    post_res = client.post(
        "/api/v1/posts/", json={"title": "Single", "content": "Content"}, headers=headers
    )
